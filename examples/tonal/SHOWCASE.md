...
choice makes a whole family of tonal phenomena fall out — two of them genuinely **impossible**
...
multiply-linked autosegment) and can carry **contour tones** (several tones on one vowel). The
notation is in `docs/change_notation_rules.md` §1.8–§1.9 (tier references and lexical floating
tones) and §2.12 (the spread / dock / delink operations); the engine internals are in the
project `README.md` under *Autosegmental tiers*.
